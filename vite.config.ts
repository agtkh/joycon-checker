...
import { VitePWA } from 'vite-plugin-pwa' // プラグインをインポート
...
export default defineConfig({
  plugins: [
    ...
    VitePWA({
      ...
      // PWAのマニフェスト設定
      manifest: {
        name: 'Nintendo Switch Controller Checker',
        short_name: 'Controller Checker',
        description: 'ブラウザで動作するNintendo Switchコントローラー（Joy-Con, Proコントローラー）の動作確認ツール。',
        theme_color: '#282c34', // アプリのテーマカラー
        background_color: '#282c34', // スプラッシュスクリーンの背景色
        display: 'standalone', // スタンドアロンアプリとして表示
        scope: '/joycon-checker/', // PWAが制御するスコープ
        start_url: '/joycon-checker/', // 起動時のURL
        "icons": [
          {
  ...
  base: '/joycon-checker/', // GitHub Pagesのベースパス
})